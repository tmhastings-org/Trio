...
struct TrioUserTypeDetector: UserTypeDetector {
    ...
    func detect(cycles: [LoopCycleData], settings: TrioSettingsProfile, userReport: MealHandlingType) -> DetectedUserType {
        guard !cycles.isEmpty else {
            return DetectedUserType(hasCarbEntries: false, hasFPUEntries: false, hasManualBoluses: false,
                                     medianCarbEntrySize: nil, uamActivePercentage: 0,
                                     inferredType: userReport, agreesWithUserReport: true, disagreementNote: nil)
        }
        ...
        return DetectedUserType(hasCarbEntries: hasCarbEntries, hasFPUEntries: hasFPU,
                                 hasManualBoluses: hasManualBoluses, medianCarbEntrySize: medianCarb,
                                 uamActivePercentage: uamPct, inferredType: inferred,
                                 agreesWithUserReport: agrees, disagreementNote: note)
    }

    private func detectFPUPattern(_ cycles: [LoopCycleData]) -> Bool {
        var inCOB = false; var depleted = false; var secondary = 0
        for c in cycles {
            let cob = c.determination.cob ?? 0
            if cob > 0 { if depleted { secondary += 1; depleted = false }; inCOB = true }
            else if inCOB { depleted = true; inCOB = false }
        }
        return secondary >= 2
    }
    ...
    private func computeMedianCarbEntry(_ cycles: [LoopCycleData]) -> Decimal? {
        var peaks: [Decimal] = []; var peak: Decimal = 0; var inMeal = false
        for c in cycles {
            let cob = c.determination.cob ?? 0
            if cob > 0 { peak = max(peak, cob); inMeal = true }
            else if inMeal { if peak > 0 { peaks.append(peak) }; peak = 0; inMeal = false }
        }
        if inMeal && peak > 0 { peaks.append(peak) }
        guard !peaks.isEmpty else { return nil }
        let s = peaks.sorted(); let m = s.count / 2
    ...
    private func compare(inferred: MealHandlingType, reported: MealHandlingType, hasCarbEntries: Bool) -> (Bool, String?) {
        if reported == .varies { return (true, nil) }
        if inferred == reported { return (true, nil) }

        switch (reported, inferred) {
        case (.carbsOnly, .noEntry), (.carbsFatProtein, .noEntry):
            return (false, "You indicated you enter carbs, but none were found in this period. Consider re-exporting after a typical 48-hour window.")
        case (.noEntry, .carbsOnly), (.noEntry, .carbsFatProtein):
            return (false, "You indicated no carb entries, but carbs were detected. Analysis uses the detected pattern.")
        default:
            return (false, "Detected (\(inferred.rawValue)) differs from reported (\(reported.rawValue)). Analysis uses detected pattern.")
        }
    }
}
